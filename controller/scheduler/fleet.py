...
CONTAINER_TEMPLATE = [
    ...
    {"section": "Service", "name": "ExecStart", "value": '''/bin/sh -c "IMAGE=$(etcdctl get /deis/registry/host 2>&1):$(etcdctl get /deis/registry/port 2>&1)/{image}; port=$(docker inspect -f '{{{{range $k, $v := .ContainerConfig.ExposedPorts }}}}{{{{$k}}}}{{{{end}}}}' $IMAGE | cut -d/ -f1); services=$(/opt/bin/kubevars); docker run --name {name} {memory} {cpu} {hostname} -P -e PORT=$port $services $IMAGE {command}"'''},  # noqa
